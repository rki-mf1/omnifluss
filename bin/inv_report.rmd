...
if (length(list.files("invalid_files") != 0)) {
  ...
  samples <- read.csv(file=params$sample_sheet)
  samples_number <- nrow(samples)
...
if (len_empty_fasta != 0) {
    ...
    cat(paste0("<span style='font-size:16px; font-weight:bold'>", len_empty_fasta,"/", samples_number, " samples failed during read extraction:<br></span>"))
...
if (len_empty_kma != 0) {
    ...
    cat(paste0("<span style='font-size:16px; font-weight:bold'>", len_empty_kma,"/",samples_number*8," potential sample-segment pairs failed during reference inference:<br></span>"))
