...
col_order <- unlist(lapply(segment_order, function(seg) { grep(paste0("\\", seg, "$"), names(dt.bamstats_coverage), value = TRUE) }))
...
for (s in c(unique(dt.top.refs.na.filled$sample))) {
  ...
  tmp.df <- tibble('sample' = head(tmp$sample,5),
                    'reference hit' = 1:5) %>% add_column(!!!ref_content)
...
dir.create(params$data_output_dir)
write.csv(x = tmp_base.content.table[,1:(length(final_cols)+2)],
  row.names = FALSE,
