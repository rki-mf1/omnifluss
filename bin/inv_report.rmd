...
params:
  ...
  num_samples: "none"
  data_output_dir: "qc_report_data"
  version: "1.0.0"
---
...
ordered_segments <- c("HA", "NA", "MP", "NP", "NS", "PA", "PB1", "PB2")
```
```{r def_functions}
...
f.color_bar <- function(
    color = "lightgray", fun = "proportion",
    ...) {
  fun <- match.fun(fun)
...
infer_segment_name <- function(header) {
  # first check the suffix of the header
  # suffix can be `__` or `|`
  # `__` will be produced by omnifluss with automatic reference selection
  #  we hope for a `|<segment>` suffix for static reference selection
  seg <- str_match(
    header,
    "(?:__|\\|)(PB2|PB1|PA|HA|NP|NA|MP|M|NS)$"
  )[,2]
  # if no matching suffix is found, try to match the segment name anywhere in the header
  # assume `|` as separator as this is default for GISAID
  if (!is.na(seg))
    return(seg)

  str_match(
    header,
    "\\|(PB2|PB1|PA|HA|NP|NA|MP|M|NS)(?:\\||$)"
  )[,2]
}

infer_sample_name <- function(x) {
  return(strsplit(x, "\\__ref_")[[1]][1])
}
...
`r if (params$information_json != "none" || length(list.files("invalid_files") != 0)) {"## Sequencing Run Information"}`
```{r sequencing_information, results="asis"}
#prints information specified in the JSON file
...
#prints information about excluded samples
if (length(list.files("invalid_files") != 0)) {


  empty_kma <- list.files(path="invalid_files",
                        pattern = "*.kma.spa$",
                        full.names = T,
                        recursive = F)

  empty_fasta <- list.files(path="invalid_files",
                          pattern = ".gz$",
                          full.names = T,
                          recursive = F)

  len_empty_fasta = length(empty_fasta)
  len_empty_kma = length(empty_kma)

#process empty fasta
if (len_empty_fasta != 0) {
    samples_empty_fasta <- unique(sapply(strsplit(sub("invalid_files/", "", empty_fasta),"\\."),'[',1))
    len_empty_fasta <- length(samples_empty_fasta)
    samples_empty_fasta <- sort(samples_empty_fasta)
    samples_empty_fasta_string <- paste(samples_empty_fasta, sep=", ")

    cat(paste0("<span style='font-size:16px; font-weight:bold'>", len_empty_fasta,"/", params$num_samples, " samples failed during read extraction:<br></span>"))
    cat(paste0("<span style='font-size:16px; font-weight:bold'>",samples_empty_fasta_string,"<br></span>"))
    cat("<br>")
  }

#process empty kma
if (len_empty_kma != 0) {
    samples_empty_kma <- sapply(strsplit(sub("invalid_files/","",empty_kma),"\\."),'[',1:2)

    parts <- strsplit(sub("invalid_files/", "", empty_kma),"\\.")
    samples_empty_kma <- sapply(parts, function(v) sprintf("%s (%s)", v[1], v[2]))

    samples_empty_kma <- sort(samples_empty_kma)
    samples_empty_kma_string <- paste(samples_empty_kma, collapse=", ")
    length(samples_empty_kma_string)

    cat(paste0("<span style='font-size:16px; font-weight:bold'>", len_empty_kma,"/", as.numeric(params$num_samples)*8," potential sample-segment pairs failed during reference inference:<br></span>"))
    cat(paste0("<span style='font-size:16px; font-weight:bold'>",samples_empty_kma_string,"<br></span>"))
    cat("<br>")
  }
}
```
...
# for mapping statistics
l.infiles.read.duplicates <- list.files(
  path = ".",
...
# for mapping statistics/coverage distribution
l.infiles.bamstats_coverage <- list.files(
  path = ".",
...
l.infiles.read.ranking.refs <- list.files(
  path = ".",
  pattern = ".kma.spa$",
  full.names = T,
  recursive = F
...
# for consensus sequence
l.infiles.fasta.files <- list.files(
  path = ".",
  pattern = ".fa$",
  full.names = T,
  recursive = F
...
df.filter.data <- ldply(l.trimming.data.json, function(e) {
  df.output <- data.frame(
    passed_filter = e$filtering_result$passed_filter_reads,
...
df.summary <- data.frame(
  sample = unique(df.trimming.data$sample),
...
dir.create(params$data_output_dir)
write.csv(
  x = df.summary,
  row.names = FALSE,
  file = file.path(params$data_output_dir, "read_statistics.csv")
)
```
`r if(length(l.infiles.trimming)!= 0){"<div class=superhighimage>"}`

```{r count_trimming_samples, eval = length(l.infiles.trimming) != 0}
sample.count <- length(unique(df.table$sample))
plot.width <- 20
```

`r if(length(l.infiles.trimming) != 0){"For a better overview, the reads counts are plotted in a bar plot per sample. The number of raw reads is shown in light blue and dark blue visualizes trimmed reads."}`

```{r plot_trimming, eval = length(l.infiles.trimming) != 0}
...
`r if(length(l.infiles.trimming) != 0){"</div>"}`

`r if(length(l.infiles.trimming) != 0){"## Read Quality Distribution\nFor the forward (R1) and reverse (R2) read the sequence quality distribution is shown after trimming."}`

```{r read_trimming_for_phred, eval = length(l.infiles.trimming) != 0}
...
`r if(length(l.infiles.trimming) != 0){"<div class=superhighimage>"}`
```{r plot_phred_score, eval = length(l.infiles.trimming) != 0}
...
```{r trimming_cleanup, eval = length(l.infiles.trimming) != 0}
rm(df.plot)
rm(df.summary)
rm(df.table)
rm(df.trimming.data)
...
write.csv(
  x = df.kraken_output,
  row.names = FALSE,
  file = file.path(params$data_output_dir, "kraken_classification.csv")
)
...
dt.coverage$chromosome <- sapply(dt.coverage$chromosome, infer_segment_name)
...
# rename chromosome column values
dt.bamstats_coverage$chromosome <- sapply(dt.bamstats_coverage$chromosome, infer_segment_name)

dt.bamstats_coverage <- reshape(dt.bamstats_coverage, idvar = "sample", timevar = "chromosome", direction = "wide")
dir.create(params$data_output_dir)
write.csv(
  x = dt.bamstats_coverage,
  row.names = FALSE,
  file = file.path(params$data_output_dir, "test_bamstats_coverage1.csv")
)
...
if (ncol(dt.bamstats_coverage) != 25) {
  for (s in ls.segment) {
    if (Reduce("|", grepl(s, names(dt.bamstats_coverage)))) {} else {
...
write.csv(
  x = dt.bamstats_coverage,
  row.names = FALSE,
  file = file.path(params$data_output_dir, "test_bamstats_coverage2.csv")
)
segment_order <- c(".HA", ".NA", ".MP", ".NP", ".NS", ".PA", ".PB1", ".PB2")
col_order <- unlist(lapply(segment_order, function(seg) {
  grep(paste0("\\", seg, "$"), names(dt.bamstats_coverage), value = TRUE)
}))
final_order <- c("sample", col_order)

# reorder dataframe columns
dt.bamstats_coverage <- dt.bamstats_coverage[, ..final_order]
...
for (s in unique(dt.coverage$sample)) {
  ...
  for (l in ls.segment) {
    if (Reduce("|", grepl(l, unique(tmp$chromosome)))) {} else {
...
df.output <- data.frame(
  "sample" = unique(df.bamstat.data$sample),
...
seg_col_range <- seq(6, ncol(dt.bamstats_coverage) + 4, by = 3) # taking every third segment column
...
write.csv(
  x = df.mapping.table,
  row.names = FALSE,
  file = file.path(params$data_output_dir, "mapping_statistics.csv")
)
...
for (s in c(sample.names)) {
  plt <- ggplot(
    dt.coverage[(dt.coverage$sample == s) & (dt.coverage$mid.bin == 50), ],
    aes(x = position, y = mean.cov, fill = chromosome, colour = chromosome)
  ) +
    labs(
      title = paste("coverage distribution of", s, sep = " "), y = "mean coverage",
      fill = "chromosome"
    ) +
...
# read file content
dt.top.refs <- as.data.table(ldply(l.infiles.read.ranking.refs, fread,
  sep = "\t",
...
dt.top.refs <- dt.top.refs %>%
  group_by(sample, segment) %>%
  arrange(desc(`magic number`))
dt.top.refs <- dt.top.refs %>% arrange(sample, segment)
dt.top.refs <- dt.top.refs[, -3]

present_segments <- unique(dt.top.refs$segment)
final_cols <- ordered_segments[ordered_segments %in% present_segments]
final_cols_taxid <- paste0("taxid_", final_cols)
...
df.top.refs.reshaped <- tibble(
  "sample" = character(),
  "reference hit" = numeric(),
  !!!setNames(lapply(final_cols_taxid, function(x) character()), final_cols_taxid)
)

for (s in c(unique(dt.top.refs.na.filled$sample))) {
  tmp <- dt.top.refs.na.filled[dt.top.refs.na.filled$sample == s, ]

  ref_content <- setNames(lapply(final_cols, function(x) {
    na.omit(tmp[tmp$segment == x, ]$taxid)
  }), final_cols_taxid)

  tmp.df <- tibble(
    "sample" = head(tmp$sample, 5),
    "reference hit" = 1:5
  ) %>% add_column(!!!ref_content)
...
dir.create(params$data_output_dir)
write.csv(
  x = df.top.refs.reshaped,
  row.names = FALSE,
  file = file.path(params$data_output_dir, "top5_references.csv")
)
```
...
df.top.refs.reshaped %>%
  ...
  formattable() %>%
  kbl(
    digits = 3, col.names = NULL, align = c("r"), format = "html",
    escape = T, linesep = NA
  ) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover"), fixed_thead = T,
    full_width = T
  ) %>%
  add_header_above(c("Sample", "Reference Hit", final_cols)) %>%
  row_spec(seq(1, nrow(df.top.refs.reshaped), 5), background = "#EEEEEE") %>%
  column_spec(1, color = "black", background = "#EEEEEE") %>%
  column_spec(3:(length(final_cols) + 2), width_min = "5in") %>%
  scroll_box(height = "400px")
```
...
final_cols <- ordered_segments[ordered_segments %in% unique(segment.names)]
...
l.res <- reshape(l.res,
  v.names = c("N", "nonBase"), timevar = "Segment", idvar = "Sample",
  direction = "wide"
)
...
l.res.count <- reshape(l.res.count,
  v.names = c("N", "nonBase"), timevar = "Segment", idvar = "Sample",
  direction = "wide"
)
```
...
base.Ncontent.table <- data.frame(
  "Sample" = l.res$Sample,
  "base_freq" = rep("% N's", n),
...
base.Ncontent.count.table <- data.frame(
  "Sample" = l.res.count$Sample,
...
base.NonBasecontent.table <- data.frame(
  "Sample" = l.res$Sample,
...
base.NonBasecontent.count.table <- data.frame(
  "Sample" = l.res.count$Sample,
...
tmp_base.content.table <- tmp_base.content.table[, names(tmp_base.content.table) %in% c("Sample", "base_freq", final_cols)]
dir.create(params$data_output_dir)
write.csv(
  x = tmp_base.content.table[, 1:(length(final_cols) + 2)],
  row.names = FALSE,
  file = file.path(params$data_output_dir, "N_content_and_Ambiguous_calls.csv")
)
...
base.content.table <- base.content.table[, names(base.content.table) %in% c("Sample", "base_freq", final_cols)]
```
```{r table_base_content, eval = length(l.infiles.fasta.files) != 0}
# https://stackoverflow.com/questions/2288485/how-to-convert-a-data-frame-column-to-numeric-type
# Format table
base.content.table[, 1:(length(final_cols) + 2)] %>% # length(final_cols)+2 = Sample, base_freq + 1 col per segment
  group_by(Sample) %>%
  slice(1:4) %>%
  select(Sample, everything()) %>%
  collapse_rows_df(Sample) %>%
  ...
  add_header_above(c("Sample" = 2, final_cols)) %>%
